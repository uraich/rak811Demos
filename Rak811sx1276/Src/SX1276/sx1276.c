...
//    SX1276.Settings.Channel = freq;
	printf("SX1276SetChannel: frequency: %ld\r\n",freq);
    freq = ( uint32_t )( ( double )freq / ( double )FREQ_STEP );
    printf("SX1276SetChannel: frequency after division by FREQ_STEP: 0x%6lx\r\n",freq);
...
#endif
	rfreq[0] = SX1276Read( REG_FRFMSB);
    rfreq[1] = SX1276Read( REG_FRFMID);
    rfreq[2] = SX1276Read( REG_FRFLSB);
    readBack = (((uint32_t)rfreq[0] << 16) | ((uint32_t)rfreq[1] << 8) | (uint32_t) rfreq[0]);
    readBack = (uint32_t) ((double)readBack * (double) FREQ_STEP);

    printf("Frequency read back: %ld\r\n",readBack);
}
#ifdef READY
...
static void RxChainCalibration( void )
{
    ...
    printf("RxChainCalibration : initial frequency: %ld\r\n",initialFreq);
...
void SX1276SetModem( RadioModems_t modem )
{
    ...
    {
    	printf("SX1276SetModem: Set modem to LoRa\r\n");
        SX1276.Settings.Modem = MODEM_LORA;
    }
    else
    {
    	printf("SX1276SetModem: Set modem to FSK\r\n");
 ...
 * SPI data transfer
 */
uint16_t SpiInOut( SPI_HandleTypeDef *spi, uint16_t outData )
{
   uint8_t rxData = 0;
...
//   BoardDisableIrq( );
   ...
   spi->Instance->DR = ( uint16_t ) ( outData & 0xFF );

   while( __HAL_SPI_GET_FLAG( spi, SPI_FLAG_RXNE ) == RESET );
   rxData = ( uint16_t ) spi->Instance->DR;
...
void SX1276WriteBuffer( uint8_t addr, uint8_t *buffer, uint8_t size )
{
    ...
    //NSS = 0;
#ifdef DEBUG
    printf("SX1276WriteBuffer: data size: %d\r\n",size);
...
//    GpioWrite( &SX1276.Spi.Nss, 0 );
    ...
    if (HAL_SPI_Transmit(&hspi1,&address,1,5) != HAL_OK) {
    	printf("SX1276WriteBuffer: Could not send address\r\n");
    	return;
    }
//    SpiInOut( &SX1276.Spi, addr | 0x80 );
    if (HAL_SPI_Transmit(&hspi1,buffer,size,5) != HAL_OK) {
    	printf("SX1276WriteBuffer: Could not send buffer\r\n");
    	return;
    }
...
//    GpioWrite( &SX1276.Spi.Nss, 0 );
#ifdef DEBUG
    printf("SX1276ReadBuffer: data size: %d\r\n",size);
#endif
    HAL_GPIO_WritePin(RADIO_NSS_GPIO_Port, RADIO_NSS_Pin, GPIO_PIN_RESET);

    if (HAL_SPI_Transmit(&hspi1,&address,1,5) != HAL_OK) {
    	printf("SX1276ReadBuffer: Could not send address\r\n");
    	return;
    }
    if (HAL_SPI_Receive(&hspi1,buffer,size,5) != HAL_OK) {
    	printf("SX1276ReadBuffer: Could not send address\r\n");
    	return;
    }
