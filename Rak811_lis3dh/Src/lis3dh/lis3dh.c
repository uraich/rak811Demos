...
bool LIS3DH_ReadReg(uint8_t Reg, uint8_t* Data)
{
	...
	retCode = HAL_I2C_Mem_Read(&hi2c1, LIS3DH_MEMS_I2C_ADDRESS, Reg, 1, Data, 1, 100);

#ifdef DEBUG
	printf("LIS3DH_ReadReg: reg_addr= 0x%02x\r\n", Reg);
#endif
	if (retCode != HAL_OK) {
...
uint8_t LIS3DH_WriteReg(uint8_t Reg, uint8_t Data)
{
	...
	retCode =  HAL_I2C_Mem_Write( &hi2c1, 0x32, Reg, 1, (uint8_t *)&Data, 1, 100 );
#ifdef DEBUG
	printf("LIS3DH_WriteReg: reg_addr = 0x%02x, data = 0x%2x\r\n", Reg, Data);
#endif
	if (retCode !=HAL_OK) {
		printf("LIS3DH no ack\r\n");
	}
...
#ifdef DEBUG
  printf("LIS3GH_Init: read 0x%02x from WHO_AM_I register\r\n",whoami);
#endif

  // Enable Temp
  LIS3DH_SetTemperature((State_t) MEMS_ENABLE);
...
status_t LIS3DH_SetTemperature(State_t state)
{
  ...
  if( !LIS3DH_ReadReg(LIS3DH_TEMP_CFG_REG, &value) ) {
	  printf("LIS3DH_SetTemperature: Error reading LIS3DH_TEMP_CFG register\r\n");
	  return MEMS_ERROR;
  }
#ifdef DEBUG
  printf("LIS3DH_SetTemperature: value read from LIS3DH_TEMP_CFG_REG: 0x%02x\r\n",value);
#endif
  value &= 0x3F;
  value |= state << LIS3DH_TEMP_EN;
  value |= state << LIS3DH_ADC_EN;
  
  if( !LIS3DH_WriteReg(LIS3DH_TEMP_CFG_REG, value) )
    return MEMS_ERROR;
#ifdef DEBUG
  printf("LIS3DH_SetTemperature: value written to LIS3DH_TEMP_CFG_REG: 0x%02x\r\n",value);
#endif
  return MEMS_SUCCESS;
}
...
status_t LIS3DH_GetTempRaw(int8_t* buff)
{
	...
	if( !LIS3DH_ReadReg(LIS3DH_OUT_3_L, &valueL) ) {
		printf("LIS3DH_GetTempRaw: Error reading LIS3DH_OUT_3_L\r\n");
		return MEMS_ERROR;
	}
	if( !LIS3DH_ReadReg(LIS3DH_OUT_3_H, &valueH) ) {
		printf("LIS3DH_GetTempRaw: Error reading LIS3DH_OUT_3_H\r\n");
		return MEMS_ERROR;
	}
#define DEBUG
#ifdef DEBUG
	printf("LIS3DH_GetTempRaw: valueH:0x%02x, valueL: 0x%02x\r\n",valueH,valueL);
	printf("LIS3DH_GetTempRaw: valueH:%d, valueL: %d\r\n",(int8_t)valueH,(int8_t)valueL);
#endif
#undef DEBUG
	*buff = (int8_t)( valueH + LIS3DH_TEMPERATURE_OFFSET);
...
status_t LIS3DH_SetAxis(LIS3DH_Axis_t axis)
{
  ...
  if( !LIS3DH_ReadReg(LIS3DH_CTRL_REG1, &value) )
    return MEMS_ERROR;
#ifdef DEBUG
  printf("LIS3DH_SetAxis: read 0x%02x from LIS3DH_CTRL_REG1\r\n",value);
#endif
  value &= 0xF8;
  value |= (0x07 & axis);
  ...
  if( !LIS3DH_WriteReg(LIS3DH_CTRL_REG1, value) )
    return MEMS_ERROR;
#ifdef DEBUG
  printf("LIS3DH_SetAxis: send 0x%02x to LIS3DH_CTRL_REG1\r\n",value);
#endif
  return MEMS_SUCCESS;
}
...
* Function Name  : LIS3DH_GetAccAxesRaw
* Description    : Read the Acceleration Values Output Registers
*                : Find out the resolution and right adjust the Axes values
* Input          : buffer to empity by AxesRaw_t Typedef
* Output         : None
* Return         : Status [MEMS_ERROR, MEMS_SUCCESS]
*******************************************************************************/
status_t LIS3DH_GetAccAxes(AxesRaw_t* buff)
{
	AxesRaw_t lis3dhAxes;
	status_t retCode;
	uint8_t lis3dhResolution;

	if ((retCode = LIS3DH_GetAccAxesRaw(&lis3dhAxes)) != MEMS_SUCCESS)
		return retCode;
	if ((retCode = LIS3DH_ReadReg(LIS3DH_CTRL_REG1,&lis3dhResolution)) != MEMS_SUCCESS)
		return retCode;
	if ((lis3dhResolution & LIS3DH_LPEN)) {
	/* low power and low 8 bit resolution mode */
		buff->AXIS_X = lis3dhAxes.AXIS_X >>= 8;
		buff->AXIS_Y = lis3dhAxes.AXIS_Y >>= 8;
		buff->AXIS_Z = lis3dhAxes.AXIS_Z >>= 8;
		return retCode;
	}

	if ((retCode = LIS3DH_ReadReg(LIS3DH_CTRL_REG4,&lis3dhResolution)) != MEMS_SUCCESS)
		return retCode;
	if ((lis3dhResolution & LIS3DH_HR)) {
		/* high 12 bit resolution mode */
		buff->AXIS_X = lis3dhAxes.AXIS_X >>= 4;
		buff->AXIS_Y = lis3dhAxes.AXIS_Y >>= 4;
		buff->AXIS_Z = lis3dhAxes.AXIS_Z >>= 4;
		return retCode;
	}
	/* normal 10 bit resolution */
	buff->AXIS_X = lis3dhAxes.AXIS_X >>= 6;
	buff->AXIS_Y = lis3dhAxes.AXIS_Y >>= 6;
	buff->AXIS_Z = lis3dhAxes.AXIS_Z >>= 6;
	return retCode;
}
/*******************************************************************************
